...
public class MainActivity extends Activity {
  EditText etToken, etRepo;
  TextView tvLog, tvStatus, tvVal;
  Button btnDown, btnBuild;
  Uri zipUri=null;
  ...
  @Override protected void onCreate(Bundle b){
    ...
    r.setBackgroundColor(0xFFF5F5F5);
    TextView t=new TextView(this); t.setText("SENE FABRICA V22 FIX"); t.setTextSize(22); t.setTextColor(0xFFA0233D); t.setTypeface(null,1);
    etToken=new EditText(this); etToken.setHint("Token ghp_..."); etToken.setBackgroundColor(-1);
    etRepo=new EditText(this); etRepo.setHint("Nombre ej: mi-app"); etRepo.setBackgroundColor(-1);
    Button pick=new Button(this); pick.setText("1. SELECCIONAR ZIP"); pick.setBackgroundColor(0xFF607D8B); pick.setTextColor(-1);
    tvVal=new TextView(this); tvVal.setText("Validador: esperando ZIP"); tvVal.setPadding(20,20,20,20); tvVal.setBackgroundColor(0xFFEEEEEE);
    ...
    btnDown=new Button(this); btnDown.setText("DESCARGAR APK COMPILADA"); btnDown.setBackgroundColor(0xFF4CAF50); btnDown.setTextColor(-1); btnDown.setTextSize(18); btnDown.setVisibility(8);
    tvLog=new TextView(this); tvLog.setText("Log:");
    ScrollView sv=new ScrollView(this); sv.addView(tvLog);
    r.addView(t); r.addView(etToken); r.addView(etRepo); r.addView(pick); r.addView(tvVal); r.addView(btnBuild); r.addView(tvStatus); r.addView(btnDown); r.addView(sv);
    setContentView(r);
    ...
    btnBuild.setOnClickListener(v->{
      ...
      String rep=etRepo.getText().toString().trim();
      if(tok.isEmpty()||rep.isEmpty()){ toast("Falta token/nombre"); return; }
      btnDown.setVisibility(8);
  ...
  void validar(Uri uri){
    try{
      ...
      ZipEntry e; while((e=zis.getNextEntry())!=null){ set.add(e.getName()); zis.closeEntry(); }
      zis.close();
  ...
  void crear(String token,String owner,String repo) throws Exception{
    ...
    String js="{\"name\":\""+repo+"\",\"private\":false}";
    Request rq=new Request.Builder().url("https://api.github.com/user/repos").header("Authorization","Bearer "+token).post(RequestBody.create(js,MediaType.parse("application/json"))).build();
    client.newCall(rq).execute();
    setS("Subiendo ZIP",0xFF03A9F4);
    InputStream is=getContentResolver().openInputStream(zipUri);
    ByteArrayOutputStream ba=new ByteArrayOutputStream();
    byte[] bf=new byte[8192]; int n; while((n=is.read(bf))!=-1) ba.write(bf,0,n);
    String b64=Base64.encodeToString(ba.toByteArray(),0);
    ...
    Request disp=new Request.Builder().url("https://api.github.com/repos/"+owner+"/"+repo+"/actions/workflows/compilar.yml/dispatches").header("Authorization","Bearer "+token).post(RequestBody.create("{\"ref\":\"main\"}",MediaType.parse("application/json"))).build();
    client.newCall(disp).execute();
    setS("Compilando 3 min...",0xFFFFFF00);
    for(int i=0;i<20;i++){
      Thread.sleep(20000);
      setS("Compilando "+((i+1)*20)+"s",0xFFFFFF00);
      try{
        Request ch=new Request.Builder().url("https://api.github.com/repos/"+owner+"/"+repo+"/releases/latest").header("Authorization","Bearer "+token).get().build();
        Response cr=client.newCall(ch).execute();
        if(cr.isSuccessful()){
          ...
          if(jo.has("assets")){
            ...
            if(ar.length()>0){
              ...
              setS("APK LISTA - Toca DESCARGAR",0xFF4CAF50);
              runOnUiThread(()-> btnDown.setVisibility(0));
              log("Lista: "+apkUrl);
              return;
            }
          }
  ...
  void put(String token,String owner,String repo,String path,String b64,String msg) throws Exception{
    ...
    Request g=new Request.Builder().url(url).header("Authorization","Bearer "+token).get().build();
    Response rg=client.newCall(g).execute();
    String sha=null;
    if(rg.isSuccessful()){
      String bd=rg.body().string();
      int idx=bd.indexOf("\"sha\":\"");
      if(idx!=-1){ int s2=idx+7; int e=bd.indexOf("\"",s2); sha=bd.substring(s2,e); }
    }
    String js="{\"message\":\""+msg+"\",\"content\":\""+b64+"\",\"branch\":\"main\""+(sha!=null?",\"sha\":\""+sha+"\"":"")+"}";
    Request p=new Request.Builder().url(url).header("Authorization","Bearer "+token).put(RequestBody.create(js,MediaType.parse("application/json"))).build();
    client.newCall(p).execute();
  }
}
